...
.sf-student-name  { font-size: 13px; font-weight: 600; }
.sf-student-grade { font-size: 11px; color: var(--muted); margin-top: 1px; }
...
.sf-step.done   .sf-step-num { background: var(--cyan-dim); border-color: var(--cyan); color: var(--cyan); }
.sf-step.done .sf-step-num::after { content: '✓'; }
.sf-step.done .sf-step-num { font-size: 0; }
.sf-step.done .sf-step-num::after { font-size: 11px; }
.sf-step-label { font-size: 13px; font-weight: 600; color: var(--muted); transition: color .15s; }
...
/* ── Right form area ── */
.sf-form-area { background: var(--ink); padding: 48px 52px; overflow-y: auto; }

.sf-section { display: none; animation: sfFadeIn .25s ease both; }
...
.sf-original span {
  background: var(--faint); padding: 1px 7px; border-radius: 4px;
...
.sf-btn-submit {
  ...
  border: 1px solid var(--cyan); font-weight: 800;
}
.sf-btn-submit:hover {
...
<div class="sf-wrap">
  ...
  <aside class="sf-panel">
    <a href="{{ route('students.show', $student) }}" class="sf-back">
      <i class="bi bi-arrow-left"></i> Back to Student
    </a>

    <span class="sf-edit-badge"><i class="bi bi-pencil"></i> Editing</span>

    <div class="sf-panel-icon"><i class="bi bi-person-badge"></i></div>
    <div class="sf-panel-eyebrow">Edit record</div>
    <h1 class="sf-panel-title">Edit Student</h1>

    {{-- Student identity chip --}}
    <div class="sf-student-chip">
      <div class="sf-student-avatar">
        {{ strtoupper(substr($student->first_name, 0, 1)) }}{{ strtoupper(substr($student->last_name, 0, 1)) }}
      </div>
      <div>
        <div class="sf-student-name">{{ $student->full_name }}</div>
        <div class="sf-student-grade">{{ $student->admission_number }} · Grade {{ $student->grade }}</div>
      </div>
    </div>
      ...
      <section class="sf-section active" id="step-1">
        ...
        <div class="sf-grid sf-grid-2">
          <div class="sf-field">
            ...
            <div class="sf-original"><i class="bi bi-clock-history" style="font-size:11px;"></i> Original: <span>{{ $student->admission_number }}</span></div>
        ...
        <div class="sf-grid sf-grid-3">
          ...
          <div class="sf-field">
            <label class="sf-label">Gender <span class="req">*</span></label>
            <select name="gender" class="sf-input {{ $errors->has('gender') ? 'is-invalid' : '' }}" required>
              <option value="">Select…</option>
              <option value="male"   {{ old('gender', $student->gender) == 'male'   ? 'selected' : '' }}>Male</option>
              <option value="female" {{ old('gender', $student->gender) == 'female' ? 'selected' : '' }}>Female</option>
            </select>
            @error('gender')<span class="sf-error"><i class="bi bi-exclamation-circle"></i>{{ $message }}</span>@enderror
          </div>
        </div>

        <div class="sf-nav">
          <a href="{{ route('students.show', $student) }}" class="sf-btn sf-btn-ghost">
            <i class="bi bi-x-lg"></i> Cancel
          </a>
          <button type="button" class="sf-btn sf-btn-next" onclick="goToStep(2)">
            Next: Guardian Details <i class="bi bi-arrow-right"></i>
          </button>
        </div>
      </section>
          ...
          <div class="sf-field">
            ...
            <div class="sf-input-wrap">
              ...
              <input type="text" name="guardian_name"
                     ...
                     class="sf-input {{ $errors->has('guardian_name') ? 'is-invalid' : '' }}"
                     placeholder="Jane Kamau" required>
            </div>
            @error('guardian_name')<span class="sf-error"><i class="bi bi-exclamation-circle"></i>{{ $message }}</span>@enderror
          </div>
          <div class="sf-field">
            ...
            <div class="sf-input-wrap">
              ...
              <input type="tel" name="guardian_phone"
                     ...
                     class="sf-input {{ $errors->has('guardian_phone') ? 'is-invalid' : '' }}"
                     placeholder="+254 700 000 000" required>
            </div>
            @error('guardian_phone')<span class="sf-error"><i class="bi bi-exclamation-circle"></i>{{ $message }}</span>@enderror
          </div>
        </div>

        <div class="sf-grid sf-grid-2">
          <div class="sf-field">
            <label class="sf-label">Guardian Email</label>
            <div class="sf-input-wrap">
              <i class="bi bi-envelope sf-input-icon"></i>
              <input type="email" name="guardian_email"
                     value="{{ old('guardian_email', $student->parent_email) }}"
                     class="sf-input {{ $errors->has('guardian_email') ? 'is-invalid' : '' }}"
                     placeholder="parent@email.com">
            </div>
            @error('guardian_email')<span class="sf-error"><i class="bi bi-exclamation-circle"></i>{{ $message }}</span>@enderror
          </div>
          <div class="sf-field">
            <label class="sf-label">Relationship</label>
            <select name="guardian_relationship" class="sf-input">
              <option value="">— Select —</option>
              @foreach(['Mother','Father','Guardian','Grandparent','Sibling','Aunt','Uncle','Other'] as $rel)
      ...
      <section class="sf-section" id="step-3">
        <div class="sf-section-heading">
          ...
          <div>
            ...
            <div class="sf-section-sub">Academic year, enrollment date and status</div>
          </div>
        </div>
        ...
        <div class="sf-grid sf-grid-3">
          ...
          <div class="sf-field">
            <label class="sf-label">Status <span class="req">*</span></label>
            <select name="status" class="sf-input {{ $errors->has('status') ? 'is-invalid' : '' }}" required>
            ...
            @error('status')<span class="sf-error"><i class="bi bi-exclamation-circle"></i>{{ $message }}</span>@enderror
          </div>
        </div>
        ...
        <div class="sf-nav">
          <div class="d-flex gap-2">
            ...
            <button type="button" class="sf-btn sf-btn-delete"
                    onclick="if(confirm('Permanently delete {{ $student->full_name }}? This cannot be undone.')) document.getElementById('deleteForm').submit()">
...
(function(){
  ...
  @if($errors->any())
    ...
    const step3Fields = ['academic_year','enrollment_date','status'];
    let start = 1;
    if (errorFields.some(f => step3Fields.includes(f))) start = 3;
    else if (errorFields.some(f => step2Fields.includes(f))) start = 2;
    goToStep(start);
  @endif
